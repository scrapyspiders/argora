...
function Timeline({
  ...
  const requestLastPosts = useCallback(async () => {
    try {
      console.log(`Calling requestLastPosts : `, new Date().getMinutes(), new Date().getSeconds());
      setLoading(true);
      setPosts([]);
      ...
      Promise.all(contents).then((results) => {
        ...
        if (componentTracker.current === planet) {
          ...
          if (planet && lastPosts.length > 0) savePlanet(planet);
        }
        componentTracker.current = planet;
      });
    } catch (e) {
  ...
  useEffect(() => {
    const fetchData = async () => await requestLastPosts();
  ...
  return (
    <>
      {type === 'main' && <h1 style={{ textAlign: 'center' }}>{planet ? `Planet 🪐 ${planet}` : 'The Metaweave'}</h1>}
      {((type === 'profile' && txid === walletAddr) || type !== 'profile') && (
        <Form
          type={type}
      ...
      {!loading && !error && posts && posts.length < 1 ? (
        ...
        posts?.map((post, i) => (
          <div key={i}>
            ...
            {post.time ? <Link to={`/${pathBase}/thread/${post.id}`}>{displayPost(post)}</Link> : displayPost(post)}
          </div>
        ))
      )}
    </>
